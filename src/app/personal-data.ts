export interface PersonalData {
    name: string;
    profession: string;
    address: string;
    avatar: string;
    bio: string;
    education: Education[];
    experience: Experience[];
    skills: Skills[];
    languages: Languages[];
    social: Social[];
}

export interface Education {
    degree: string;
    description: string;
    institution: string;
    endDate: string;
}

export interface Experience {
    company: string;
    startDate: string;
    endDate: string;
    jobTitle: string;
    responsibilities: string[];
}

export interface Skills {
    name: string;
    percentage: string;
}

export interface Languages {
    name: string;
    proficiency: string;
}

export interface Social {
    name: string;
    url: string;
}

export const DATA = {
    "name": "Cristian Mosquera",
    "profession": "BackEnd Developer",
    "address": "Medellin, Colombia.",
    "avatar": "https://mypicture.png",
    "bio":`I'm an Electronic engineering student and a web developer focused on backend. I have a great passion for
    building systems and I focus on deliver clean, robust, and testable software as possible. I keep learning about
    how to deliver high scalable and maintainable software.
    I also know the basics of artificial intelligence and IoT, but I’m very interested in going deeper
    into these subjects.`,
    "education": [
      {
        "degree": "Electronic Engineering",
        "description": "",
        "institution": "University Of Antioquia",
        "startDate": "2011",
        "endDate": "Present",
      },
      {
        "degree": "Mechatronics Technician",
        "description": "",
        "institution": "Sena",
        "startDate": "2011",
        "endDate": "2013",
      }
    ],
    "experience": [
      {
        "company": "Globant.",
        "startDate": "December 2019",
        "endDate": "Present",
        "jobTitle": "Java Developer",
        "responsibilities": [
          'Support the UM Team at Disney',
        ],
      },
      {
        "company": "Ceiba Software.",
        "startDate": "July 2019",
        "endDate": "December 2014",
        "jobTitle": "Full Stack Developer",
        "responsibilities": [
          'Support the development of the Wingo airline website to make flight reservations.',
          'Built a scan packing frontend with angular 7 to keep track of clothing inventory.',
        ],
      },
      {
        "company": "Corantioquia/UdeA.",
        "startDate": "September 2018",
        "endDate": "January 2019",
        "jobTitle": "Instrumentation",
        "responsibilities": [
          'Installation of IoT stations to measure the weather conditions of the department of Antioquia.',
          'I Optimized the code of the weather stations (python + raspberry pi, IoT) in order to decrease the raspberry pi temperature by 30% and the CPU usage by 20%.',
        ],
      },
      {
        "company": "Cabitech.",
        "startDate": "March 2017",
        "endDate": "May 2018",
        "jobTitle": "Freelance Web Developer",
        "responsibilities": [
          'Built an admin web application to manage the products of vending machines.',
          'Developed a restful API to sell cell phone recharges.',
          'Developed an IoT prototype to communicate several devices on tables in a restaurant. Using nodejs and arduino with the ESP8266 Wifi module and the MQTT protocol.'
        ],    
      }
    ],
    "skills": [
      {
        "name": "Backend development with Java 8/11.",
        "percentage": "75%"
      },
      {
        "name": "Restful APIs with the Spring framework (Core/MVC/Boot).",
        "percentage": "70%"
      },
      {
        "name": "Angular 7+.",
        "percentage": "80%"
      },
      {
        "name": "TDD with JUnit, Mockito and Protractor.",
        "percentage": "60%"
      },
      {
        "name": "OOP, SOLID design principles, and Design Patterns.",
        "percentage": "80%"
      },
      {
        "name": "Version control with Git.",
        "percentage": "70%"
      },
      {
        "name": "Relational databases (MySQL, PostgreSQL).",
        "percentage": "70%"
      },
      {
        "name": "Hibernate ORM.",
        "percentage": "70%"
      },
      {
        "name": "Dependency management with Maven and NPM.",
        "percentage": "60%"
      },
      {
        "name": "Javascript.",
        "percentage": "80%"
      },
      {
        "name": "HTML.",
        "percentage": "90%"
      },
      {
        "name": "CSS.",
        "percentage": "65%"
      },
      {
        "name": "AWS",
        "percentage": "60%"
      },
    ],
    "languages": [
      {
        "name": "Spanish",
        "proficiency": "native"
      },
      {
        "name": "English",
        "proficiency": "B2"
      }
    ],
    "social": [
      {
        "name": "linkedin",
        "url": "https://www.linkedin.com/in/cristian-mosquera-escobar-0377ab186"
      },
      {
        "name": "github",
        "url": "https://github.com/cmow5"
      }
    ]
  }